...
function getForwardPath(req: IncomingMessage): string {
  const incoming = new URL(req.url || '/api', 'http://vercel.local');
  const rawPath = incoming.searchParams.get('path') || '/';
  incoming.searchParams.delete('path');

  const parsedPath = new URL(rawPath, 'http://upstream.local');
  const pathname = parsedPath.pathname.startsWith('/') ? parsedPath.pathname : `/${parsedPath.pathname}`;

  const merged = new URLSearchParams(parsedPath.searchParams);
